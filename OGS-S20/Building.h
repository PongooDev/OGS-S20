...
namespace Building {
	...
	void Hook() {
		// Only OnDamageServer Works?!
		MH_CreateHook((LPVOID)(ImageBase + 0x739B08C), ServerCreateBuildingActor, nullptr);
		//HookVTable(AAthena_PlayerController_C::GetDefaultObj(), 0x230, ServerCreateBuildingActor, nullptr);

		HookVTable(AAthena_PlayerController_C::GetDefaultObj(), 0x25B, ServerBeginEditingBuildingActor, nullptr);

		HookVTable(AAthena_PlayerController_C::GetDefaultObj(), 0x259, ServerEndEditingBuildingActor, nullptr);

		MH_CreateHook((LPVOID)(ImageBase + 0x739B664), ServerEditBuildingActor, nullptr);
